...
# raster_columns
a <- dbReadTable(con, dbListTables(con)[1])
head(a)
saveRDS(a, "raster_columns.rds", compress= "xz")

# raster_overviews
a <- dbReadTable(con, dbListTables(con)[2])
head(a)
saveRDS(a, "raster_overviews.rds", compress= "xz")

# spatial_ref_sys
a <- st_read(con, dbListTables(con)[3])
head(a)
saveRDS(a, "raster_overviews.rds", compress= "xz")

# geography_columns
a <- dbReadTable(con, dbListTables(con)[4])
head(a)
saveRDS(a, "geography_columns.rds", compress= "xz")

# ESQUEMAS geometry_columns ####
...
# base_radares ####
a <- st_read(con, dbListTables(con)[8])
head(a)
