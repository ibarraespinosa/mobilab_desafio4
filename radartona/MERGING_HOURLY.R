...
speedss <- list.files(path = "/media/sergio/ext4/MOBILAB/speeds", 
                    pattern = ".rds", 
                    full.names = TRUE)
radars <- list.files(path = "/media/sergio/ext4/MOBILAB/rds", 
                      pattern = ".rds", 
                      full.names = TRUE)
...
for(i in 1:length(dias)){
  for(j in 1:length(hora)){
    ...
    radar <- readRDS(paste0("/media/sergio/ext4/MOBILAB/rds/sao_roque_", fecha))
    ...
    saveRDS(speeds_cont_radar2, 
            paste0("/media/sergio/ext4/MOBILAB/merged/",fecha))
  }
}
